...
interface ConversationListProps {
  ...
  resyncToken?: number;
  /**
   * Like `resyncToken`, but bumped (debounced) by the parent every time
   * a realtime message INSERT lands — so a new message re-fetches the
   * list and the conversation that received it surfaces on top via the
   * server's `last_message_at` ordering. Separate from `resyncToken` so
   * the message-thread pane isn't refetched on every inbound.
   */
...
export function ConversationList({
  ...
  useEffect(() => {
    ...
    // `messageRefreshToken` does the same on each (debounced) inbound
    // message so a new message re-orders the list to the top server-side.
  }, [resyncToken, messageRefreshToken, profileLoading, user?.id, isAgent]);
  ...
  const filtered = useMemo(() => {
    ...
    // Sort by most-recent activity so a conversation jumps to the top the
    // instant a new message lands. Realtime message INSERTs patch
    // `last_message_at` in place (see handleMessageEvent in the inbox page)
    // but never reorder the array — the DB fetch's initial ordering goes
    // stale the moment the first live message arrives. Sorting here, on the
    // render path, keeps the freshest thread pinned to the top no matter
    // which update path (message INSERT, conv UPDATE, hydrate) touched it.
    // Copy first: `result` may still be the `conversations` prop and
