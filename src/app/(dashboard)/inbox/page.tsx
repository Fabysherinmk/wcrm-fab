...
export default function InboxPage() {
  ...
  /**
   * Dedicated refresh signal for the ConversationList only (the thread
   * pane deliberately does NOT listen to this — refetching the open
   * thread on every unrelated inbound would churn its scroll position).
   * Bumped, debounced, whenever a realtime message INSERT lands so the
   * list re-fetches from the DB — which returns rows ordered by
   * `last_message_at` desc — guaranteeing the conversation that just
   * received a message floats to the top even if an in-place realtime
   * patch was missed or arrived out of order. The instant in-place patch
   * in handleMessageEvent still runs for snappy UX; this is the
   * authoritative catch-up behind it.
   */
  const [convListRefreshToken, setConvListRefreshToken] = useState(0);
  const convListRefreshTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  const { user, profileLoading, isAgent } = useAuth();

  // Coalesce bursts (a flow reply fires several message INSERTs back to
  // back) into a single list refetch ~600ms after the last one lands.
  const scheduleConvListRefresh = useCallback(() => {
    if (convListRefreshTimerRef.current) {
      clearTimeout(convListRefreshTimerRef.current);
    }
    convListRefreshTimerRef.current = setTimeout(() => {
      setConvListRefreshToken((n) => n + 1);
  ...
  const handleMessageEvent = useCallback(
    (event: { eventType: string; new: Message; old: Partial<Message> }) => {
      ...
      if (event.eventType === "INSERT") {
        ...
        // Authoritative catch-up: re-fetch the list (server-ordered by
        // last_message_at desc) so the conversation that just received a
        // message is guaranteed to surface on top, debounced so a burst
        // of flow replies triggers a single refetch.
        scheduleConvListRefresh();
      }
    ...
    [activeConversation, hydrateConversation, scheduleConvListRefresh]
  );
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        >
          <ConversationList
            ...
            messageRefreshToken={convListRefreshToken}
          />
        </div>
